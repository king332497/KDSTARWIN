(() => {
  ...
  const state={csrf:'',admin:null,permissions:new Set(),users:[],routes:[],selectedChat:null,events:null,typingTimer:null,audio:localStorage.getItem('kb_admin_audio')!=='0',audioCtx:null,audioReady:false,recentSounds:new Map()};
  const views=[
  ...
  const getAudioContext=()=>{
    ...
    if(!AC)return null;
    try{state.audioCtx=new AC();return state.audioCtx}catch{return null}
  };
  const unlockAudio=async()=>{
    const c=getAudioContext();if(!c)return;
    try{if(c.state==='suspended')await c.resume();state.audioReady=c.state==='running'}catch{}
  };
  const soundOnce=(key,fn,ttl=2500)=>{
    const now=Date.now(),last=state.recentSounds.get(key)||0;
    if(now-last<ttl)return;
    state.recentSounds.set(key,now);fn();
    if(state.recentSounds.size>80){for(const [k,t] of state.recentSounds){if(now-t>30000)state.recentSounds.delete(k)}}
  };
  const playNotification=(kind='chat')=>{
    if(!state.audio)return;
    const c=getAudioContext();if(!c)return;
    const run=()=>{
      const start=c.currentTime+.01;
      const notes=kind==='entry'?[[620,0,.10],[820,.12,.14]]:[[760,0,.08],[960,.09,.11]];
      notes.forEach(([freq,offset,duration])=>{
        const o=c.createOscillator(),g=c.createGain();
        o.type='sine';o.frequency.setValueAtTime(freq,start+offset);
        g.gain.setValueAtTime(.0001,start+offset);
        g.gain.exponentialRampToValueAtTime(.055,start+offset+.012);
        g.gain.exponentialRampToValueAtTime(.0001,start+offset+duration);
        o.connect(g);g.connect(c.destination);o.start(start+offset);o.stop(start+offset+duration+.02);
      });
    };
    if(c.state==='suspended')c.resume().then(()=>{state.audioReady=true;run()}).catch(()=>{});else{state.audioReady=true;run()}
  };
  ...
  function connectEvents(){
    ...
    es.addEventListener('chat.message',ev=>{
      ...
      if(m.sender_type==='USER'){
        soundOnce(`chat:${m.id||m.user_id}:${m.created_at||''}`,()=>{toast(`Pesan baru dari ${m.sender_name||'User'}`);playNotification('chat')});
      }
      if(state.selectedChat===m.user_id)selectChat(m.user_id).catch(()=>{});
      refreshAll().catch(()=>{});
    });
    es.addEventListener('session.started',ev=>{
      const d=JSON.parse(ev.data);
      soundOnce(`session:${d.session_id||d.user_id}`,()=>{
        const u=state.users.find(x=>x.id===d.user_id);
        toast(`User masuk: ${u?.full_name||'User baru'}`);
    ...
    ['presence.updated','user.updated','user.created'].forEach(e=>es.addEventListener(e,()=>refreshAll().catch(()=>{})));
  }
  ...
  $('#audioToggle').addEventListener('change',async e=>{state.audio=e.target.checked;localStorage.setItem('kb_admin_audio',state.audio?'1':'0');if(state.audio){await unlockAudio();playNotification('entry')}toast(`Audio notification ${state.audio?'aktif':'nonaktif'}`)});
