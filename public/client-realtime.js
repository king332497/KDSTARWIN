(() => {
  ...
  const ensureIdentity = () => new Promise(resolve => {
    if(state.user?.full_name) return resolve();
    const modal=document.createElement('div'); modal.id='kbIdentityGate';
    modal.style.cssText='position:fixed;inset:0;z-index:99998;background:rgba(23,23,26,.48);backdrop-filter:blur(8px);display:grid;place-items:center;padding:18px;font-family:Inter,system-ui,sans-serif';
    modal.innerHTML=`<form style="width:min(430px,100%);background:#fff;border-radius:24px;padding:24px;box-shadow:0 30px 90px rgba(0,0,0,.2)"><div style="width:40px;height:5px;border-radius:99px;background:#ffcb05;margin-bottom:18px"></div><h2 style="margin:0;font-size:1.35rem">Identitas sesi</h2><p style="color:#6c6c74;line-height:1.55;font-size:.9rem">Masukkan nama lengkap agar identitas yang sama digunakan pada website, Live Chat, session, dan Admin Panel.</p><label style="display:block;font-weight:700;font-size:.78rem;margin:18px 0 7px">Nama lengkap</label><input name="full_name" autocomplete="name" maxlength="100" required minlength="3" style="width:100%;box-sizing:border-box;min-height:48px;border:1px solid #ddd;border-radius:13px;padding:0 13px;font:inherit;font-size:16px" placeholder="Nama lengkap"><button style="width:100%;min-height:48px;margin-top:13px;border:0;border-radius:13px;background:#ffcb05;font:inherit;font-weight:800;cursor:pointer">Lanjutkan</button><div data-error style="min-height:18px;margin-top:8px;color:#b42318;font-size:.78rem"></div></form>`;
    document.body.appendChild(modal);
    const f=$('form',modal), err=$('[data-error]',modal);
    f.addEventListener('submit',async ev=>{ ev.preventDefault(); const full_name=new FormData(f).get('full_name'); try{ const data=await api('/api/user/identity',{method:'POST',body:JSON.stringify({full_name})}); state.user={...state.user,...data}; modal.remove(); applyIdentity(); resolve(); }catch(e){err.textContent=e.message;} });
  });
  const applyIdentity = () => {
  ...
  (async()=>{
    try{
      ...
      await ensureIdentity(); installPresence(); connectEvents();
