(() => {
  ...
  const nodes = () => ({
    ...
    card: $('[data-kb-hero-card]') || $('.hero-campaign-v6 .campaign-card-v6'),
    image: $('[data-kb-hero-dynamic]') || $('.hero-campaign-v6 .campaign-card-v6 img'),
  ...
  async function applyHeroContent() {
    ...
    try {
      ...
      if (n.image) {
        ...
        currentUrls = (
          Array.isArray(hero.image_urls) && hero.image_urls.length
            ? hero.image_urls
          ...
          const showSlide = index => {
            ...
            preload.onload = () => {
              const reveal = () => {
                ...
                if (n.card) n.card.hidden = false;
                n.image.style.opacity = '1';
              };

              n.image.style.transition = reduceMotion ? 'none' : 'opacity .38s ease';

              if (reduceMotion || n.image.hidden || !n.image.getAttribute('src')) {
                reveal();
              } else {
                n.image.style.opacity = '0';
                setTimeout(reveal, 210);
              }
            };

            preload.onerror = () => {
              if (!n.image.getAttribute('src')) {
                n.image.hidden = true;
                if (n.card) n.card.hidden = true;
              }
            };
